<?php


class Cart extends Product_Details
{

    public function  update_cart(){
        global  $db;
        $error_message = '';
        if(isset($_POST['update_cart'])) {
            $i = 0;
            $statement = "SELECT * FROM " . self::$table_product ;
            $exec = $db->exec_query($statement);


            while ($result = mysqli_fetch_assoc($exec)) {
                $i++;
                $table_product_id[$i] = $result['p_id'];
                $table_quantity[$i] = $result['p_qty'];


            }


            $i=0;
            foreach($_POST['product_id'] as $val) {
                $i++;
                $arr1[$i] = $val;
            }
            $i=0;
            foreach($_POST['quantity'] as $val) {
                $i++;
                $arr2[$i] = $val;
            }
            $i=0;
            foreach($_POST['product_name'] as $val) {
                $i++;
                $arr3[$i] = $val;
            }

            $allow_update = 1;
            $temp_index = '';
            for($i=1;$i<=count($arr1);$i++) {
                for($j=1;$j<=count($table_product_id);$j++) {
                    if($arr1[$i] == $table_product_id[$j]) {
                        $temp_index = $j;//number the quantity in table

                    }
                }
                if($table_quantity[$temp_index] >= $arr2[$i]) {
                    $_SESSION['cart_p_qty'][$i] = $arr2[$i];
                }
            }
            header('location: shopping-cart.php');

            ?>


            <?php







        }





    }
    public  function  delete_item_cart(){
        // Check if the product is valid or not


        $i=0;
        foreach($_SESSION['cart_p_id'] as $key => $value) {
            $i++;
            $arr_cart_p_id[$i] = $value;
        }

        $i=0;
        foreach($_SESSION['cart_size_id'] as $key => $value) {
            $i++;
            $arr_cart_size_id[$i] = $value;
        }

        $i=0;
        foreach($_SESSION['cart_size_name'] as $key => $value) {
            $i++;
            $arr_cart_size_name[$i] = $value;
        }

        $i=0;
        foreach($_SESSION['cart_color_id'] as $key => $value) {
            $i++;
            $arr_cart_color_id[$i] = $value;
        }

        $i=0;
        foreach($_SESSION['cart_color_name'] as $key => $value) {
            $i++;
            $arr_cart_color_name[$i] = $value;
        }

        $i=0;
        foreach($_SESSION['cart_p_qty'] as $key => $value) {
            $i++;
            $arr_cart_p_qty[$i] = $value;
        }

        $i=0;
        foreach($_SESSION['cart_p_current_price'] as $key => $value) {
            $i++;
            $arr_cart_p_current_price[$i] = $value;
        }

        $i=0;
        foreach($_SESSION['cart_p_name'] as $key => $value) {
            $i++;
            $arr_cart_p_name[$i] = $value;
        }

        $i=0;
        foreach($_SESSION['cart_p_photo'] as $key => $value) {
            $i++;
            $arr_cart_p_featured_photo[$i] = $value;
        }

        unset($_SESSION['cart_p_id']);
        unset($_SESSION['cart_size_id']);
        unset($_SESSION['cart_size_name']);
        unset($_SESSION['cart_color_id']);
        unset($_SESSION['cart_color_name']);
        unset($_SESSION['cart_p_qty']);
        unset($_SESSION['cart_p_current_price']);
        unset($_SESSION['cart_p_name']);
        unset($_SESSION['cart_p_photo']);

        $k=1;
        for($i=1;$i<=count($arr_cart_p_id);$i++) {
            if( ($arr_cart_p_id[$i] == $_REQUEST['id']) && ($arr_cart_size_id[$i] == $_REQUEST['size']) && ($arr_cart_color_id[$i] == $_REQUEST['color']) ) {
                continue;
            } else {
                $_SESSION['cart_p_id'][$k] = $arr_cart_p_id[$i];
                $_SESSION['cart_size_id'][$k] = $arr_cart_size_id[$i];
                $_SESSION['cart_size_name'][$k] = $arr_cart_size_name[$i];
                $_SESSION['cart_color_id'][$k] = $arr_cart_color_id[$i];
                $_SESSION['cart_color_name'][$k] = $arr_cart_color_name[$i];
                $_SESSION['cart_p_qty'][$k] = $arr_cart_p_qty[$i];
                $_SESSION['cart_p_current_price'][$k] = $arr_cart_p_current_price[$i];
                $_SESSION['cart_p_name'][$k] = $arr_cart_p_name[$i];
                $_SESSION['cart_p_photo'][$k] = $arr_cart_p_featured_photo[$i];
                $k++;
            }
        }




    }






}
$cart = new Cart();